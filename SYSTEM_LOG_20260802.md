...
- **数据健康标志不入库（07-31 缺口 #17，未修未结转过，此处补上）**：audit dict 23 键（22 键 + src_round，classify.py:244-272 实查）仍无 warmup/stale/实际排位窗深 win 任何健康位——回测无法区分「完整 250 窗口下算的行」与「90 根 warmup 期的行」。补键属 **AUDIT_VERSION 升版级变更**，回测按数据质量分层依赖它（见第 7 节 P0）。
