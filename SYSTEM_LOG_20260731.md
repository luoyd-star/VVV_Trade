...
> **⚠️ 已有后继版本：`SYSTEM_LOG_20260805.md`**（覆盖 2026-08-01 → 08-05，72 个提交；
> 宇宙 10→74 品种、个股 IV 换源 moomoo、波动率栈四层、RULES_VERSION v3.1、回测框架落成）。
> 本文不废弃——它记录的是 v1 的诚实状态，后继版本只在结论变了的地方明写"旧版说 X，现在是 Y"。
> **查系统现状请以 20260805 版为准**；本文档只作为 v1 的历史切片。
>
...
> **【2026-08-05】X2 扩容 38→74 + 财报条件分位。**
> 扩容原则=补已实测盲区（张成检验：科技 R²=0.938 几乎完全张成，公用 0.044/
> 能源 0.061/必需消费 0.148/医疗 0.151 结构性失明），故名额给低 R² 板块而非
> 再加 QCOM/TXN 那类只加厚 0.94 维度的名字。新增贵金属 XAG/XPT/XPD、
> 加密 BNB/XRP/DOGE（只取币安有期权者，sources 钉 binance_futures）、美股 30。
> 验收 74/74 三表齐备零遗漏；治理快照记录 37→38→74。
> **财报条件分位**（docs/EARNINGS_IV_CONTAMINATION_20260804.md 续章）：
> 实测财报对 IV30 的影响是**阶跃而非斜坡**（0-30 天 1.20~1.27，31-35 天骤降 1.02，
> 阶跃恰在 30 天=跳跃进出 IV30 计价窗口处，幅度 +30.9%）。因此污染项由提前公布的
> 日历完全决定、不含未知量——**不需期权链反解，且前向使用无障碍**。
> 解法是把分位参照集限定在同一 in30 状态，实测富集 1.92×→0.94×。
> 同批修两个真 bug：3304 返回当日未结算值（会污染分母，已按 NYSE 真实收盘时刻过滤）；
> 财报邻近度系统性偏移一日（比时间戳而非比日期；测试因与 bug 共享"两端锚在午夜"
> 的假设而未能抓到，已改用真实 ET 盘中时刻）。
>
...
## 附录：波动率栈分层批次（2026-08-04 ~ 08-05）

两天内完成的一整批：个股 IV 换源、宇宙翻倍、规则升版、波动率前端层。细节在 docs/ 逐题报告，此处只记"现在什么为真"。

### 交付清单（详情见对应文档）

| 件 | 一句话 | 文档 |
|---|---|---|
| 个股 IV 换源 | moomoo 3303/3304 取代"29/31 拿 VXN 当自己 IV"；22,390 行 / 3.1 年；`stock_vol` source 进主键；settled/live 双轨，分位只算 settled | RESEARCH_IV_PERSTOCK / PLATFORMS_20260804 |
| 财报条件化 | 事件前 10 日占 IV 分位≥0.8 的 35.2%（样本占比仅 15.4%）→ 条件分位（EARN_WINDOW_D=30, rank_kind cond/raw 并存） | EARNINGS_IV_CONTAMINATION_20260804 |
| X2 扩容 | 38→74 品种，名额按张成检验补盲区（公用 R²=0.044 等）而非加厚科技 | X2_EXPANSION_20260805 |
| RULES_VERSION v3.1 | A1 事件感知确认门槛：事件窗（ET 日历日差∈[0,10] 含当天）内 squeeze→趋势确认 2→3 根；回测 48 事件 81% 良性拦截；exp_event_gate 挂周复跑再评估 | A1_EVENT_GATE_20260805 |
| 两轮 codex 审阅 | 6 路 + 12 路共 ~190 条发现全部独立复核；真 P0 全部出自端到端数值对拍路 | AUDIT_CODEX_20260805 / _R2_20260805 |
| 加密近端 IV | opt_iv_near 扩到 8 标的（+BTC/ETH/SOL/BNB/XRP/DOGE），24/7 每 30 分钟；总方差插值 ~3d 常数期限（DOGE 2.31d 单点如实标注） | — |
| 个股期限曲线 | `stock_iv_term`：61 品种 3d/9d/30d ATM（同行权价 C/P 均值），RTH 每小时；期权链按 ET 日缓存（每日约 183 次请求），盘中每轮仅 2 次批量快照 | — |

### 波动率四层架构（8-05 定型）与用户裁决

- **3d 持仓前端**（加密 opt_iv_near / 美股 iv3）：直接计价 1-3 天持仓窗的风险。
- **9d 中段**（美股 iv9 / VIX9D）：事件隔离带——事件在 30d 窗内、9d 窗外时，9d<30d 的抬升段即日历定价（首采 NVDA 9d=38.2 < 30d=44.4，正是 8/26 财报）。
- **30d 制度层**（DVOL / iv30）：唯一有深历史（3.1 年）可算分位的层，制度判定继续用它。
- **事件轴**（earnings + known_at）：条件分位与 v3.1 确认门槛的输入。

**用户裁决（8-05）**：三层同等重要，**3d 交易价值最高**；历史自当日起自攒（美股 RTH 纯口径——盘后短期权报价噪音大，测试行已清；加密 24/7）。3d 分位转正列 GAPS B6/B7 第一顺位，约 60 交易日后可做粗分位。

### 3d 层图表化 + 近端 IV 估计器稳健化（08-05 午间追加）

- **IV vs RV 卡升级为 30d+3d 双层四线**：DVOL/个股IV（蓝）+ RV30（琥珀）+ 3d 隐含（金实线）+ RV3（金虚线，72×1h 年化）。meta 加 `RV3` 与 `3dIV−RV3`（持仓期限口径的剪刀差）；Hermes 简报同步。RV3 历史即刻可用（~100 天），IV3 自攒、在图右缘生长。
- **近端 IV 估计器稳健化（第一天就翻了车，幸而当天修）**：首版"单 ATM 行权价 C/P 均值"实测被单点坏 mark 击穿——XAU 35 分钟 23.7→38.2→31.0，XAG 瞬时 87.5；拉链检查发现 XAG 0.75d 到期一半报价钉在 110.0（占位值），2.75d 正常簇 32-44 里混着 88.5 坏点。修法：占位簇剔除（同值 ≥3 次）→ ATM 邻域 5 行权价全部报价 → 中位数。修后 20 秒复探漂移 <0.25pt。首日 44 行旧口径数据按"升版即重算"清零重攒。
- stock_iv_term 的 ATM 取法与首版同构（单行权价对），列 GAPS C15 观察两周，出现尖峰即照搬邻域中位数。

### 运维事实

- 采集守护重启于 08-05 09:49 JST，新旧代码切换期间日志里"币安近端IV 只有 XAU/XAG"的行均来自旧进程，非缺陷。守护/面板于 12:00 JST 再次重启（估计器 + 3d 图表批）。
- 42 个循环"错误"仍是 X2 新永续 1d 不足 90 根的 warmup 告警（宁缺毋滥闸正常工作，随时间自愈）。
- series_floor_{symbol} meta：公司行动断点治理（CRWD 4:1 重定价，floor=2026-07-02 13:00 UTC）；backfill_history 尚不读 floor。
