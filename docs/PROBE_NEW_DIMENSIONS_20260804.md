...
## put/call 共线性体检（8 品种 × 779 日，已回填数据实测）

价值判断前先问"是不是在重复测已有的东西"：

| 相关对 | 均值 |
|---|---|
| ρ(put/call, IV) 水平 | **+0.080** |
| ρ(Δput/call, ΔIV) | **+0.084** |
| ρ(put/call, 20日动量) | −0.195 |
| ρ(Δput/call, 当日收益) | −0.153 |

**与 IV 几乎完全正交**——它不是隐波的另一种测法。与动量的 −0.195 是预期中的
"价弱则买权需求上升"，但强度只有 R²≈4%，也说明它不是动量的代理。

⚠️ **正交是必要条件不是充分条件**：一个与所有已有指标都不相关的序列，也可能只是
正交的噪音。增量价值必须由样本外检验确立，本表只排除"共线所以无用"这一条否决理由。

## 已考察并否掉

**用 moomoo 的 `underlying_price` 补美股日线历史——否。** 诱因很实：美股品种当前 1d
只有 120-131 根（面板"1d 预热中 仅120根"就是它），而 moomoo 序列自带 779 个日收盘。
但两条否决理由：① 它是**正股收盘**，我们的 ohlcv 是**币安永续**，写进同一张表会触犯
跨源日界锚点纪律（见系统日志缺口 #5）；② 真要补参考历史，`ref_daily` 里 QQQ/COIN/MSTR
的 Yahoo 序列已有 **5.3-6.6 年**，比 moomoo 的 3.1 年更深，管线也现成。
**结论：美股日线该走 Yahoo 扩容（现有路径），不是 moomoo。** 此项另立，不混进本次升级。

`underlying_price` 仍有一个正当用途：与币安永续收盘对照，量化永续对正股的跟踪偏离
（基差质量体检）。属可选项，未建。
