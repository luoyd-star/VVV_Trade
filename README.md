...
| `high_vol_chop` 高波动无序 | 波动率极高但没有方向效率，常见于顶底转换 | ATR 分位 > 0.85 且不成趋势 |
| `range` 震荡 | 以上都不是 | 兜底 |

判定优先级：趋势 > 挤压 > 高波动无序 > 震荡。`confidence` 表示子信号的一致程度（0–1）。
...
**审计化快照**：每行随存 `features`（22 项特征值 JSON）、`rules`（触发/未满足规则清单）与
...
| 波动率 | `regime/features/volatility.py` | ATR 及其相对价格的历史分位、布林带宽分位、30 根年化已实现波动率；**波动率加速度**（快RV12/慢RV72，>1 扩张中）与**下行方差占比**（近 48 根，>0.5 跌出来的波动）。核心思想：波动率绝对值无意义，**自身历史分位**才可比。 |
