...
判定优先级：趋势 > 挤压 > 高波动非趋势 > 震荡。`confidence` 是 **raw_state 对应的状态专属启发式强度分**（0–1，最终保留两位），不是跨状态同构的概率或统一“一致程度”：

- 趋势态：`clip(0.45×|direction| + 0.35×er_rank + 0.20×vol_confirm)`；顺趋势量差 `>0.10 / >-0.10 / 其余` 时 `vol_confirm=1 / 0.5 / 0`
- `squeeze`：`clip(1-bbw_rank)`
- `high_vol_chop`：`clip(atr_rank)`
- `range`：`clip(0.5×(1-er_rank) + 0.5×(1-|direction|))`

**非对称迟滞（2026-07-31 起）**：`regime_history.raw_state` 存逐根原始判定，`state` 存确认态——
一般状态连续 2 根确认进入、恢复震荡需 3 根、高波冲击立即进入；v3.1 另规定美股财报窗内
`squeeze→trend_up/trend_down` 需 3 根确认。未确认的切换以"候选状态 (n/need)"暴露给面板与 VVVhermes。

**审计化快照**：每行随存 `features`（键集随 `AUDIT_VERSION` 演进，当前 a8 为 25 项）、`rules`（触发/未满足规则清单）与
...
| 路径几何（影子） | `regime/features/pathgeom.py` | **不参与判定**：chop_freq（线性去趋势残差每 100 根的均值穿越次数，区分震荡的"慢摆动 vs 快噪声"）、dom_period（主导周期）、Kendall τ（秩趋势，与 direction 交叉验证）；另有 margin（到最近**可实际翻转输出**的决策边界距离，<0.15 亮"边界过渡中"，是候选状态的领先互补）与 lag_bars 滞后声明。源自 regime-spectrum 评审采纳批次一；进规则之日必须升 RULES_VERSION，由版本谓词触发涉改序列原地重算。测试：`tests/test_pathgeom.py`（含输出级因果断言）。 |
...
`instruments.json` 声明品种类别与数据源路由；当前默认宇宙共 **74 品种**：61 个美股永续、
7 个加密、5 个商品、1 个国际股票永续。属性每次调用热读；collector 的成员列表只在启动时解析，
因此新增/删除品种成员后须重启 collector。未登记 symbol 回退到加密默认。
NVDA / AAPL / TSLA / MU / SOXL / SPY / QQQ 是 2026-07-31 的首批历史记录，不再代表当前宇宙全量。
...
- **标的休市标注**：正股仅 9:30-16:00 ET 交易而合约 24/7——休市期波动塌陷可能产生"假挤压"。
  面板顶栏显示 标的盘中/休市 徽章，VVVhermes 上下文自动带该状态与解读警示。日历为
  **2025–2027 显式 NYSE 假日与半日市表；表外年份视为休市，每年 12 月须补下一年**
- 上市较新标的（1d 不足 90 根，如 SOXL）该周期暂缺，数据攒够自动出现；90–280 根之间由 warmup 标志提示

### 美股波动率与 session 效应（现行波动率栈）

美股永续没有 DVOL 式免费长历史；现行主线已从短史 CBOE 自采切到 moomoo：
...
- **指数 IV（`regime/usvol.py` + usvol 表）**：VIX/VXN/RVX/VIX9D/VIX3M。usvol 表
...
- **个股 IV 主线（`stock_vol`, source=`moomoo`）**：已回填 2023-06-26 起的 **3.10 年**日频历史，
  当前覆盖 63 个品种；分位只在 moomoo 单一口径内计算。
- **CBOE 个股 iv30 影子/短史对照**：`delayed_quotes` 的 `iv30` 存 `deriv.iv30`，仅用于短史对账或回退，
  不再是个股 IV 主能力，也不与 moomoo 历史混拼。
- **期限与盘中层**：`stock_iv_term` 采 3d/9d/30d 曲线，`stock_vol_live` 保存盘中未结算 IV；
  面板并列指数 IV vs RV30、近端 IV vs RV3，以及 9D/30D 急性端与 30D/3M 制度端期限结构。
- **合规节流**：CBOE delayed_quotes 属 ToS 灰区，整个 usvol 模块经 meta 表节流闸限频 30 分钟。
- session/weekday 的**转正路径**（尚未动规则）：影子期收集 ds vs raw 分歧样本，
  4-6 周后评估误报率；若转正，在下一次 RULES_VERSION 升版时评审，并由版本谓词触发涉改序列原地重算。
  weekday（星期几）细分暂不做——每桶样本=周数，8-12 周后数据才够稳健估计。

## 数据

- 默认 `--sources auto` 按 `instruments.json` **逐品种路由**：当前 74 品种中 66 个钉死单源
  `binance_futures`，另 5 个走 `binance_futures→hyperliquid`；BTC/ETH/SOL 继承加密默认链。
  `Deribit→OKX→Binance` 是未登记加密品种（以及未显式配置 sources 的加密品种）的兜底链，
  不是全系统默认优先级。全部为免费公开接口、无需 API key；`--sources` 可显式覆盖。
- Deribit 注意事项：
...
- collector 从库内长历史取数并把特征计算窗固定为 400 根、分位参照窗固定为 250 根；
  独立 `main.py` CLI 仍直接取约 300 根已收盘 K 线，两条入口的窗深差异是已知限制。
...
collector.py ──写──> data/market.db (SQLite, WAL) <──读── dashboard.py ──> web/ 面板
     │                    ├ ohlcv           K 线（库内持续积累；计算窗 400、分位窗 250 均固定）
...
2. **迟滞仍未校准**：非对称 2/3 根、冲击态立即与财报窗 `squeeze→趋势` +1 根均已实现，
   但这些确认根数仍是版本化先验。
3. **入口窗深不一致**：collector 使用库内历史、400 根特征窗与 250 根分位窗；独立 CLI 仍约 300 根直拉。
   数据库序列已积累至数千根，不能再把“约 300 根”概括为全系统历史深度。
4. **IV 仅作展示**：DVOL 已进报告，但尚未作为状态机的输入特征——是否有增量、如何加权，留给回测回答。

## 路线图

- [x] 回测框架 / 协议 p3：评估 t 时确认态对 `t+1..t+H` 未来已实现波动是否具有超越无条件
      基线的预测信息，以因果 CRPS（proper score）计分；原“当期条件分布”目标因自证缺陷已否决
- [x] 数据库历史回填并持续积累；[ ] 本地 parquet 缓存
...
- [x] `warmup` / `stale` 数据健康标志贯通；[ ] `DATA_DEGRADED` 标识符从未实现，如仍需要须先定义语义
...
- [x] 美股波动率栈：去季节化 ATR 影子 + CBOE 指数 IV + moomoo 3.10 年个股 IV 主线 + CBOE 个股 iv30 影子 + 期限/盘中层
...
- [ ] 死盘外生日历剔除 + EMA 种子 k≥1.5n 规则（下一次 RULES_VERSION 升版时评审，由版本谓词触发涉改序列原地重算）
