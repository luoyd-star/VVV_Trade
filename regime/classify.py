...
#   同样的结构，短历史品种被系统性压低趋势判定概率；截至 2026-08-05，
#   现库 win<200 的行占 7.62%（v1→v2 时的历史比例见 git blame）。
...
# 它没有证明"无序"（截至 2026-08-05，该态 chop_freq 均值较 range 低 3.28%、
# 中位数低 9.16%），也没有证明"方向未证"（35.45% 的该态行方向腿
# |dir|>=0.30 已通过，只是 ER 腿没过）。
